...
runmain <- function(cutoff, naa, choice, annotationFile, pdbDirectory, updateProgress = NULL){
  ...
  if(choice == "Mode 1"){ninc <- 1/length(pdbID)}
  else if(choice == "Mode 2"){ninc <- 0.4/length(pdbID)}
  else if(choice == "Mode 3"){ninc <- 0.2/length(pdbID)}

  ###################################

  # update progress bar
  pval <- 0.1
  updateProgress(value=pval)
...
if(choice == "Mode 2" | choice == "Mode 3"){
    ...
    if(choice == "Mode 3"){ninc <- 0.4/length(alldirs)}else{ninc <- 0.6/length(alldirs)}
